...
class CrudSymptoms {
    ...
    public function createData($symptomName, $description, $note) {
        try {
            $stmt =$this->dbConn->prepare("Call AddSymptom(:symptomName, :description, :note);");
            $stmt->execute([':symptomName' => $symptomName, ':description' => $description, ':note' => $note]);
            return 1;
