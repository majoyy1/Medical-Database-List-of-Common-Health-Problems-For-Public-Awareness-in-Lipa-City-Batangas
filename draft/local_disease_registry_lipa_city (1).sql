...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 13, 2025 at 01:37 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddDisease` (IN `dName` VARCHAR(60), IN `description` TEXT, IN `classification` VARCHAR(35), IN `categoryID` INT, IN `note` TEXT)  DETERMINISTIC INSERT INTO disease(Disease_Name, Description, Classification, Category_ID, Note) 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `ModifyDisease` (IN `DName` VARCHAR(60), IN `descriptionDat` TEXT, IN `class` VARCHAR(40), IN `CId` INT, IN `note` TEXT)   UPDATE disease SET Disease_Name = DName, Description = descriptionDat, Classification = class, Category_ID = CId, Note = note 
...
(23, 'qwqw', 'qwq', 'SAsas', 1, '2025-04-13 08:25:51', ''),
(24, 'lemn', 'asd', 'Infected', 4, '2025-04-13 08:26:20', '');
